  ...
  try {
    ...
    if (action === "init") {
      ...
      const initUser = id.name
        ? `We’re opening the page. It is the ${sal}. Greet ${id.name} briefly and warmly as KnowRah. Offer one helpful, specific question or suggestion to move today forward.`
        : `We’re opening the page. It is the ${sal}. Greet the user briefly and warmly as KnowRah. Invite them to share their name and offer one helpful, specific suggestion to start.`;
      ...
      await appendMessage(userId, { role: "assistant", text: reply || "Hi — I’m here with you." });
    ...
    if (action === "learn_identity" && name) {
      ...
      const reply = await chatComplete(messages);
      await appendMessage(userId, { role: "assistant", text: reply || `Beautiful, ${name}. I’ll remember.` });
    ...
    await appendMessage("debug", { role: "assistant", text: `⚠️ ${msg}` });
    return NextResponse.json({ ok: false, reply: `⚠️ ${msg}` }, { status: 500 });
  }
}
